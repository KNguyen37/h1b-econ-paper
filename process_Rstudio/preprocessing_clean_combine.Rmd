...
analysis_data <- analysis_data %>%
  mutate(
    ...
    # # Current year dependency (original measure) _ total
    # h1b_dependency_current = total_applications / total_employment,
    # 
    # # Pre-period normalized dependency (professor's recommendation) _ total 
    # h1b_dependency_preperiod = total_applications / pre_period_employment,
...
analysis_data <- analysis_data %>%
  mutate(
    industry_category = case_when(
      naics_2digit == "48-49" ~ "Transportation and Warehousing",
...
write.csv(analysis_data, paste0(base_path, "/improved_processed_data_for_analysis_April1st.csv"), row.names = FALSE)
